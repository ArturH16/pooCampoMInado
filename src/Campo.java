...
public class Campo {
	...
	public Campo() {
		...
		for (int i = 0; i < Constantes.getNUM_LINHAS(); i++) {
			for (int j = 0; j < Constantes.getNUM_COL(); j++) {
				...
				if (j < Constantes.getNUM_COL()	- 1 )matriz[i][j].adicionarVizinhos(matriz[i -1][j + 1]);
				}
				if (j > 0) matriz[i][j].adicionarVizinhos(matriz[i][j-1]);
				if (j < Constantes.getNUM_COL()	- 1 )matriz[i][j].adicionarVizinhos(matriz[i][j + 1]);
	...
	public boolean isPerdido() {
		for (int i  = 0; i < Constantes.getNUM_LINHAS(); i ++) {
			for (int j = 0; j < Constantes.getNUM_COL(); j ++) {
				if (matriz[i][j].getClicado() && matriz[i][j].getMinado()) return true;
