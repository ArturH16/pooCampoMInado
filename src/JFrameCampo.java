...
public class JFrameCampo extends JFrame implements Recomeco{
	...
	public JFrameCampo(Campo c) {
		...
		for (int i = 0; i < Constantes.getNUM_LINHAS(); i++) {
			for (int j = 0; j < Constantes.getNUM_COL(); j++) {
				...
				matrizBut[i][j].setLocation(Constantes.getTAM_ESPACO()*j,Constantes.getTAM_ESPACO()*i + Constantes.getOFFSET_SUPERIOR());
	...
	private void confIniciais() {
		...
		this.setSize(Constantes.getTAM_ESPACO() * Constantes.getNUM_COL(), Constantes.getNUM_LINHAS() * Constantes.getTAM_ESPACO() + Constantes.getOFFSET_SUPERIOR() + Constantes.getALTURA_BARRA_SUP());
		...
		this.resetButton.setLocation((Constantes.getTAM_ESPACO() * Constantes.getNUM_COL()) / 2 - Constantes.getTAM_ESPACO() / 2, Constantes.getOFFSET_SUPERIOR() - Constantes.getTAM_ESPACO() -4);
	}
	@Override
	public void reset() {
		for (int i = 0; i < Constantes.getNUM_LINHAS(); i++) {
